...
Pages: /admin/accounts · /admin/llm · /admin/presets · /admin/agent
"""
...
_PAGES = frozenset({"accounts", "llm", "presets", "agent"})
...
def _model_list(request: Request) -> list[str]:
    raw = request.session.get("llm_model_list")
...
def admin_llm_page(request: Request, db: Session = Depends(get_db)):
    ...
    return TEMPLATES.TemplateResponse(
        ...
        _shell_ctx(
            ...
            model_list=_model_list(request),
        ),
    )
...
def admin_save(
    ...
    request.session["admin_ok"] = True
    _set_flash(request, "连接与运行环境已保存", "ok")
    return _redirect("llm")
...
async def admin_llm_test(
    ...
    try:
        ...
        if res.status_code >= 400:
            ...
            request.session.pop("llm_model_list", None)
            _set_flash(
                request,
                f"连接失败 HTTP {res.status_code}" + (f"：{detail}" if detail else ""),
                "err",
            )
            return _redirect("llm")
        ...
        if isinstance(data, dict) and isinstance(data.get("data"), list):
            for item in data["data"]:
                ...
                if len(ids) >= 80:
                    break
        request.session["llm_model_list"] = ids
        msg = f"连接成功 · 模型 {model or '—'}"
        if ids:
            msg += f" · 可用 {len(ids)} 个（见下拉）"
        _set_flash(request, msg, "ok")
    except Exception as exc:
        logger.warning("llm test failed: %s", type(exc).__name__)
        request.session.pop("llm_model_list", None)
        _set_flash(request, f"连接失败：{type(exc).__name__}", "err")
    return _redirect("llm")
